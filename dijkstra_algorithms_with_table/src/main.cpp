...
void graph()
{
    ...
    for(int i=0; i<hillN-1; i++){
        printf("D(%s) |\t ",minPaths[i+1].thisnode.name4);
    }
   ...
   // printf("\tdeneme:%d,%s",minPaths[1].dist,minPaths[1].Previous.name);
    ...
    for(int i=1; i<hillN; i++){
        if(strcmp(minPaths[i].Previous.name,"NULL")==0){
            ...
            strcpy(minPaths[i].thisnode.name3,minPaths[i].thisnode.name);
            //printf("thisnode:%s\t",minPaths[i].thisnode.name3);

            //printf("\t%4s",str.name);

        }
    }
     for(int k=0; k<hillN-1; k++){
        for(int s=k+1; s<hillN; s++){
            if(minPaths[k].dist2>minPaths[s].dist2 && minPaths[s].dist2>0 ){
            strcpy(temp3.name3,minPaths[k].thisnode.name3);
            strcpy(minPaths[k].thisnode.name3,minPaths[s].thisnode.name3);
            strcpy(minPaths[s].thisnode.name3,temp3.name3);
        }}}
    for(int i=0; i<hillN-1; i++){
        for(int j=i+1; j<hillN; j++){
            ...
            strcpy(temp2.name2,minPaths[i].Previous.name2);
            strcpy(minPaths[i].Previous.name2,minPaths[j].Previous.name2);
            strcpy(minPaths[j].Previous.name2,temp2.name2);
            //
        }
        }
    }
...
printf("\n\nSTEP|     N'    |");
    for(int i=0; i<hillN-1; i++){
        printf("D(%s) |\t ",minPaths[i+1].thisnode.name3);
    }
...
printf(" %d    |",visitedN);
   ...
   for(int i=1; i<hillN; i++){
    ...
    if(minPaths[i+counter-1].dist2==0) continue;
    else
    printf("\t %d,%s",minPaths[i+counter-1].dist2,minPaths[i+counter-1].Previous.name2);
   }
    printf("\n");
   //system("Pause");
